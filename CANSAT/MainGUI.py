...
class GUI(QMainWindow):
    def __init__(self):
        ...
        self.imageLabel.setPixmap(QPixmap("ISRO_LOGO_BACK.png"))
        self.imageLabel.setScaledContents(True)
        self.imageLabel.setAlignment(Qt.AlignCenter)
        self.imageLabel.setStyleSheet("background-color: white")
        self.imageLabel.setFrameStyle(QFrame.Panel | QFrame.Sunken)
    ...
    def currentBaudrateChanged(self): print("Baud:", self.baudrateSelector.currentText())
    def currentComChanged(self): print("COM:", self.comSelector.currentText())
    ...
    # --- CSV parsing ---
    def parse_packet(self, text_line):
        try:
            ...
            def get_float(v):
                try:
                    return float(v.replace("m", "").replace("hPa", "").replace("C", "")
                                 .replace("V", "").strip())
                except:
                    return 0.0
            ...
            # Accelerometer and gyro triplets
            accel_vals = [float(x) for x in data_dict.get("ACCEL_DATA", "0|0|0").split("|")]
            gyro_vals = [float(x) for x in data_dict.get("GYRO_RATE", "0|0|0").split("|")]

            # Build normalized dict for GUI
            return {
                "team_id": data_dict.get("TEAM_ID", "0"),
                "mission_time": data_dict.get("TIME_STAMP", "00:00:00"),
                "packet_count": get_int(data_dict.get("PACKET_COUNT", "0")),
                "altitude": get_float(data_dict.get("ALTITUDE", "0")),
                "pressure": get_float(data_dict.get("PRESSURE", "0")),
                "temp": get_float(data_dict.get("TEMPERATURE", "0")),
                "volt": get_float(data_dict.get("VOLTAGE", "0")),
                "gps_time": data_dict.get("TIME_STAMP", "00:00:00"),
                "gps_lat": get_float(data_dict.get("GNSS_LAT", "0")),
                "gps_lon": get_float(data_dict.get("GNSS_LON", "0")),
                "gps_alt": get_float(data_dict.get("GNSS_ALT", "0")),
                "gps_sats": get_int(data_dict.get("GNSS_SATS", "0")),
                "air_speed": 0.0,  # not provided
                "particle": 0.0,   # not provided
                "pitch": accel_vals[0] if len(accel_vals) > 0 else 0.0,
                "roll": accel_vals[1] if len(accel_vals) > 1 else 0.0,
                "yaw": accel_vals[2] if len(accel_vals) > 2 else 0.0
            }

        except Exception as e:
            print(f"[GUI] Field parse error: {e} :: {text_line}")
            return None
    ...
    # --- Table and plots update ---
    def update_table(self, d):
        row = self.telemetryTable.rowCount()
        self.telemetryTable.insertRow(row)
        vals = [
            d["team_id"], d["mission_time"], str(d["packet_count"]),
            f"{d['altitude']:.2f}", f"{d['pressure']:.2f}", f"{d['temp']:.2f}",
            f"{d['volt']:.2f}", d["gps_time"], f"{d['gps_lat']:.6f}",
            f"{d['gps_lon']:.6f}", f"{d['gps_alt']:.2f}", str(d["gps_sats"]),
            f"{d['air_speed']:.2f}", f"{d['particle']:.0f}",
            f"{d['pitch']:.1f}", f"{d['roll']:.1f}", f"{d['yaw']:.1f}"
        ]
        for col, val in enumerate(vals):
            self.telemetryTable.setItem(row, col, QTableWidgetItem(val))
        self.telemetryTable.scrollToBottom()
        if self.telemetryTable.rowCount() > 1000:
            self.telemetryTable.removeRow(0)

    def update_labels(self, d):
        self.timeText.setText(f"Mission Time: {d['mission_time']}")
        self.altitudeText.setText(f"Altitude: {d['altitude']:.2f} m")
        self.pitchRollYawText.setText(
            f"Pitch: {d['pitch']:.1f}°    Roll: {d['roll']:.1f}°    Yaw: {d['yaw']:.1f}°"
        )
        self.latitudeLongitudeAltitudeSatsText.setText(
            f"Lat: {d['gps_lat']:.6f}  Long: {d['gps_lon']:.6f}  Alt: {d['gps_alt']:.1f} m  SATs: {d['gps_sats']}"
        )

    def update_plots(self, d):
        try:
            t_val = float(d["packet_count"])
        except Exception:
            t_val = float(self._pkt_counter)
        self.buf_t.append(t_val)
        self.buf_alt.append(d["altitude"])
        self.buf_temp.append(d["temp"])
        self.buf_pres.append(d["pressure"])
        self.buf_airs.append(d["air_speed"])
        self.buf_volt.append(d["volt"])
        self.buf_part.append(d["particle"])
        self.curve_alt.setData(self.buf_t, self.buf_alt)
        self.curve_temp.setData(self.buf_t, self.buf_temp)
        self.curve_pres.setData(self.buf_t, self.buf_pres)
        self.curve_airs.setData(self.buf_t, self.buf_airs)
        self.curve_volt.setData(self.buf_t, self.buf_volt)
        self.curve_part.setData(self.buf_t, self.buf_part)
